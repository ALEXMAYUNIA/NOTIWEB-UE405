...
def mostrar_pagina():
    ...
    modo = st.radio("Selecciona modo:", ["📁 Carpeta automática (VIOLENCIA FAMILIAR)", "📤 Subir archivos (para Licenciada)"], horizontal=True, key="vf_modo")

    lista_df = []
    archivos = []
    
    if modo == "📤 Subir archivos (para Licenciada)":
    ...
    st.success(f"✅ Archivos leídos: {len(lista_df)} de {len(archivos)} | Total: {len(df)} casos")

...
with st.sidebar:
    ...
    st.caption("Selecciona módulo:")

    # Espacio flexible para empujar cerrar sesion abajo
    st.markdown('<div style="flex:1; height:40vh;"></div>', unsafe_allow_html=True)

    st.markdown("---")
    if st.button("Cerrar sesión", use_container_width=True, type="secondary"):
        st.session_state['logado'] = False
        st.rerun()
    st.markdown('<div style="text-align:center; font-size:10px; color:#94a3b8; margin-top:8px;">UE 405 - 2026</div>', unsafe_allow_html=True)
